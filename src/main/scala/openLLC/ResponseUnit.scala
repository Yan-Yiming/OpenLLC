...
class ResponseUnit(implicit p: Parameters) extends LLCModule with HasCHIOpcodes {
  ...
  // only for AMO by now
  def handleTxnData(ncbwrdata: Valid[RespWithData]): Unit = {
  ...
  txrsp.bits.chiOpcode := Mux(
    ...
    txrspArb.io.out.bits.chiOpcode === AtomicLoad_CLR || // watch CLR
